...
class Detect(object):
    ...
    def bullishabandonedbaby(self, df):
        # 1. 前8根趨勢為負
        # 2. 第8根為下跌
        # 3. 第8根長度在前50根中PR值小於等於40
        # 4. 第9根為十字星
        # 5. 第10根為上漲
        # 6. 第10根長度在前50根中PR值小於等於40
        # 7. 第9根開盤價小於第8根收盤價
        # 8. 第9根開盤價小於第10根開盤價
        cond1 = (df['trend8'].iloc[-3] < 0)
        cond2 = (df['direction'].iloc[-3] < 0)
        cond3 = (df['realbody_per'].iloc[-3] <= 40)
        cond4 = (df['open'].iloc[-2] == df['close'].iloc[-2])
        cond5 = (df['direction'].iloc[-1] > 0)
        cond6 = (df['realbody_per'].iloc[-1] <= 40)
        cond7 = (df['open'].iloc[-2] < df['close'].iloc[-3])
        cond8 = (df['open'].iloc[-2] < df['open'].iloc[-1])
        if cond1 and cond2 and cond3 and cond4 and cond5 and cond6 and cond7 and cond8:
            return True
        else:
            return False  

    def bearishabandonedbaby(self, df):
        # 1. 前8根趨勢為正
        # 2. 第8根為上漲
        # 3. 第8根長度在前50根中PR值小於等於40
        # 4. 第9根為十字星
    ...
    def threeblackcrows(self, df):
        ...
        # 8. 第10根為下跌
        # 9. 第10根長度大於上影線和下影線加總
        cond1 = (df['trend8'].iloc[-4] > 0)
        cond2 = (df['direction'].iloc[-3] < 0)
        cond3 = (df['realbody_per'].iloc[-3] >= (df['lshadow_width'].iloc[-3] + df['ushadow_width'].iloc[-3]))
        cond4 = (df['open'].iloc[-2] < df['open'].iloc[-3])
        cond5 = (df['direction'].iloc[-2] < 0)
        cond6 = (df['realbody_per'].iloc[-2] >= (df['lshadow_width'].iloc[-2] + df['ushadow_width'].iloc[-2]))
        cond7 = (df['open'].iloc[-1] < df['open'].iloc[-2])
        cond8 = (df['direction'].iloc[-1] < 0)
        cond9 = (df['realbody_per'].iloc[-1] >= (df['lshadow_width'].iloc[-1] + df['ushadow_width'].iloc[-1]))
        if cond1 and cond2 and cond3 and cond4 and cond5 and cond6 and cond7 and cond8  and cond9:
            return True
        else:
            return False

    def threewhitesoldiers(self, df):
        # 1. 前7根趨勢為負 
        # 2. 第8根為上漲
        # 3. 第8根長度大於上影線和下影線加總
        # 4. 第9根開盤價大於第8根開盤價
    ...
    def process(self):
        ...
        self.data['BullishAbandonedBaby'] = 0
        self.data['BearishAbandonedBaby'] = 0
        self.data['ThreeBlackCrows'] = 0
        self.data['ThreeWhiteSoldiers'] = 0
    ...
    def signal(self):
        ...
        for idx in tqdm(self.data.index):
            ...
            if start_idx >= 0:
                ...
                elif self.bullishabandonedbaby(df):
                    self.data.loc[end_idx, 'BullishAbandonedBaby'] = 1
                elif self.bearishabandonedbaby(df):
                    self.data.loc[end_idx, 'BearishAbandonedBaby'] = 1
                elif self.threeblackcrows(df):
                    self.data.loc[end_idx, 'ThreeBlackCrows'] = 1
                elif self.threewhitesoldiers(df):
                    self.data.loc[end_idx, 'ThreeWhiteSoldiers'] = 1
    ...
    def result(self):
        ...
        print('BullishAbandonedBaby: {}'.format(self.data.loc[self.data['BullishAbandonedBaby'] == 1, 'BullishAbandonedBaby'].shape[0]))
        print('BearishAbandonedBaby: {}'.format(self.data.loc[self.data['BearishAbandonedBaby'] == 1, 'BearishAbandonedBaby'].shape[0]))        
        print('ThreeBlackCrows: {}'.format(self.data.loc[self.data['ThreeBlackCrows'] == 1, 'ThreeBlackCrows'].shape[0]))
        print('ThreeWhiteSoldiers: {}'.format(self.data.loc[self.data['ThreeWhiteSoldiers'] == 1, 'ThreeWhiteSoldiers'].shape[0]))

if __name__ == "__main__":
    # 先前處理過的csv檔
    filename =  r'C:/Users/Allan Ngo/Desktop/HW1_r08723042_吳豐光/data/eurusd_2010_2017.csv'
    ...
    #　儲存經偵測過的csv檔
    filename_save = r'C:/Users/Allan Ngo/Desktop/HW1_r08723042_吳豐光/data/eurusd_2010_2017.csv'
